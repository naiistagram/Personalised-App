...
function contentFor(key: ReminderKey): { title: string; body: string } {
  switch (key) {
    case 'morningAffirmation':
      return { title: '🌸 Your affirmation is ready', body: 'Open Cloud to read today\'s belief before the day gets loud.' };
    case 'learningNudge': {
      const pick = learningLibrary[Math.floor(Math.random() * learningLibrary.length)];
      const topic = topicMeta[pick.topic];
      return {
        title: `${topic.emoji} Time for a 2-minute lesson`,
        body: `"${pick.title}" — ${topic.label}`,
      };
    }
    case 'meditation':
      return { title: '🧘‍♀️ Pause + breathe', body: 'A few minutes to meditate or pray, just for you.' };
    case 'eveningJournal':
...
export async function sendTestNotification() {
  ...
  await Notifications.scheduleNotificationAsync({
    content: {
      title: '☁️ This is what a reminder feels like',
